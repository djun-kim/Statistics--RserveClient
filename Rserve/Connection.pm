...
sub new() {
  ...
  eval {
    $inet_addr = inet_aton($host) or
      die (Rserve::Exception->new("Can't resolve host $host"));
  ...
  if ($@) {
    die (Rserve::Exception->new("Unable to create socket<pre>".$@->getErrorMessage()."</pre>"));
  }
  ...
  if ($@) {
    die Rserve::Exception->new("Unable to connect".$@->getErrorMessage());
  }
...
sub command() {
  ...
  eval {
    ...
    #print "n = $n\n";
    die Rserve::Exception->new("Invalid (short) response from server:$!") if ($n == 0);
  };
  if ($@) {
  ...
  eval {
    #print "receiving pkt..\n";
    (defined(recv($self->{socket}, $buf, 16, 0)) && length($buf) >= 16) or
